...
test('Assigning a value to a local variable.', function (trial) {
  let one   //create a variable called one
  one = 1   // assign the variable "one" the value of the number 1
...
test('What is the javascript numeric type?', function (trial) {
  trial.is('number', typeof(42));   // I don't understand this one.
})
...
test('What is NaN?', function (trial) {
  const resultOfFailedOperations = 42 / 'wat'  // resultOfFailedOperations equals a number divided by a string
...
test('How do you find the length of a string?', function (trial) {
  ...
  trial.is(5, fruit.length) // this one could be kinda misleading as both fruit and apple have 5 letters
})
...
test('What is an if statement?', function (trial) {
  const two = 2   // a variable that says two is equal to the number 2
  let isPositive = false  // counter-intuitively, isPositive is a variable that equals false
  if (two > 0) {    // if the number two is greater than zero (which it is)...
    isPositive = true   // ... isPositive equals true
  }
...
test('What is a for loop?', function (trial) {
  let counter = 10    // create a variable called counter and have it equal the number 10
  for (let i = 1; i <= 3; i++) {  // starting at 1, run if i is less than or equal to 3,
                                  
    counter = counter + i         // run code, then add 1 to i.
  }
                          // so, first run, i = 1, counter = 10, add 1 to it, add 1 to i. Now i = 2 and counter = 11
                          // second run, i = 2, counter = 11, add 2 to it, add 1 to i. Now i = 3 and counter = 13
                          // third run, i = 3, counter = 13, add 3 to it, add 1 to i. i = 4 and counter = 16
                          // loop stops because i = 4 and 4 !=< 3.
...
test('What is the type of an object?', function (trial) {
  trial.is('object', typeof({}))  // I kinda cheated on this one. I THINK what it is saying is "how do you get the
                                  // "typeof" an object? In other words, what would you type inside? Ergo, "object".
})
